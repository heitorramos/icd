...
subtitle: "Aula 07 — Módulo 1: Análise Exploratória"
...
format:
  revealjs:
    ...
    chalkboard: true
    preview-links: auto
    footer: '<span class="lecture-footer-label">ICD: Aula 07 — Tendências centrais</span><a class="lecture-footer-button" href="../aulas/07-tendencias-centrais.html">voltar para a Aula</a>'
execute:
  echo: false
---
...
![](assets/aula07-billboard/descritiva-resumo.png){.aula07-figure-large fig-alt="Resumo descritivo do dataset Billboard com número de linhas, colunas, gêneros, duração média, duração mediana e popularidade média."}
...
![](assets/aula07-billboard/descritiva-generos-ausentes.png){.aula07-figure-large fig-alt="Gráficos de barras com número de músicas por gênero e valores ausentes por coluna."}
...
## Antes de calcular: o que você espera? {.quiz-question}

Se acrescentarmos uma única faixa de **45 minutos** à base, qual resumo da
duração típica deve mudar mais?

- [A média]{.correct data-explanation="A média usa a magnitude de todos os valores; uma duração extrema desloca o seu ponto de equilíbrio."}
- [A mediana]{data-explanation="A mediana depende principalmente da ordem e da posição central, por isso tende a mudar pouco com um único extremo."}
- [As duas mudam exatamente igual]{data-explanation="Média e mediana reagem de maneiras diferentes a valores extremos."}
- [Nenhuma delas muda]{data-explanation="A nova observação entra nos dois cálculos, embora normalmente afete muito mais a média."}

## Pipeline descritivo

![](assets/aula07-billboard/pipeline-descritivo.png){.aula07-figure-large fig-alt="Pipeline descritivo em cinco etapas: escolher coluna, visualizar distribuição, calcular resumos, comparar resumos e comunicar com limites."}
...
![](assets/aula07-billboard/bins-comparacao.png){.aula07-figure-large fig-alt="Dois histogramas da duração das músicas usando poucos bins e muitos bins."}
...
2. para cada v_j, contar: c_j = número de observações com x_i <= v_j
3. calcular a frequência acumulada: F_n(v_j) = c_j / n
4. plotar (v_j, F_n(v_j)) como uma função em degraus
```
...
ecdf(4)  # fração com duração <= 4 minutos: 0.89
```

A CDF empírica transforma cada valor observado em uma pergunta acumulada:
"que fração da amostra já apareceu até aqui?".

Valores repetidos entram juntos na contagem: para cada valor distinto, usamos
a proporção de **todas** as observações menores ou iguais a ele.
...
![](assets/aula07-billboard/cdf-duracao.png){.aula07-figure-large fig-alt="CDF empírica da duração das músicas com marcação em 4 minutos."}
...
## O que significa ler 0,89 na CDF? {.quiz-question}

Suponha que a CDF empírica tenha valor **0,89** em 4 minutos. Qual interpretação
está correta?

- [89% das músicas duram exatamente 4 minutos]{data-explanation="A CDF é acumulada: ela não representa apenas as observações exatamente iguais a 4."}
- [89% das músicas duram no máximo 4 minutos]{.correct data-explanation="Por definição, Fₙ(4) é a fração observada com duração menor ou igual a 4 minutos."}
- [11% das músicas duram no máximo 4 minutos]{data-explanation="Os 11% restantes estão acima de 4 minutos; a fração acumulada até 4 é 89%."}
- [A duração média das músicas é 0,89 × 4 minutos]{data-explanation="Uma CDF descreve proporções acumuladas, não calcula a média por multiplicação."}
...
![](assets/aula07-billboard/media-mediana.png){.aula07-figure-large fig-alt="Histograma da duração com linhas verticais marcando média e mediana."}
...
![](assets/aula07-billboard/mediana-vs-media.png){.aula07-figure-large fig-alt="Comparação entre mediana e média no conjunto de valores 1, 2, 3, 4, 5, 99 e 100. A mediana é 4, no centro dos dados ordenados; a média é 30,6, puxada pelos valores extremos."}
...
![](assets/aula07-billboard/media-mediana-assimetria.png){.aula07-figure-large fig-alt="Três histogramas comparando média e mediana em uma distribuição simétrica, uma distribuição assimétrica à direita e uma distribuição assimétrica à esquerda."}
...
![](assets/aula07-billboard/outlier-media-mediana.png){.aula07-figure-large fig-alt="Barras comparando média e mediana antes e depois de adicionar uma música de 45 minutos."}
...
![](assets/aula07-billboard/quartis-duracao.png){.aula07-figure-large fig-alt="Histograma e régua ordenada da duração das músicas, com Q1, mediana e Q3 dividindo os dados em quatro partes com aproximadamente 25% das observações."}
...
![](assets/aula07-billboard/boxplot-duracao.png){.aula07-figure-large fig-alt="Boxplot horizontal da duração das músicas."}
...
![](assets/aula07-billboard/boxplot-genero.png){.aula07-figure-large fig-alt="Boxplots da duração das músicas por gênero."}
...
![](assets/aula07-billboard/quantis-cdf.png){.aula07-figure-large fig-alt="CDF empírica da duração das músicas com marcações em P10, Q1, mediana, Q3 e P90."}
...
![](assets/aula07-billboard/dispersao-genero.png){.aula07-figure-large fig-alt="Gráfico de pontos mostrando o espalhamento das durações por gênero."}
...
![](assets/aula07-billboard/variancia-comparacao.png){.aula07-figure-large fig-alt="Duas linhas de pontos com a mesma média: uma com baixa variância e valores próximos da média; outra com alta variância e valores mais afastados."}
...
## Por que aparece n − 1? {.quiz-question .quiz-multiple}

Selecione **todas** as afirmações que justificam a correção da variância
amostral.

- [Depois de estimar a média, os desvios têm apenas n − 1 graus de liberdade]{.correct data-explanation="Como os desvios em relação à média amostral somam zero, o último fica determinado pelos anteriores."}
- [Dividir por n produziria, em média, uma estimativa abaixo da variância populacional]{.correct data-explanation="A soma dos quadrados em torno da média amostral tem esperança (n−1)σ²; dividir por n mantém o viés para baixo."}
- [A correção garante que toda amostra tenha a mesma variância da população]{data-explanation="A correção remove o viés em média; uma amostra individual ainda pode produzir uma estimativa maior ou menor."}
- [Subtraímos 1 porque sempre descartamos uma observação da amostra]{data-explanation="Nenhuma observação é descartada; o que se perde é um grau de liberdade ao estimar a média."}
...
![](assets/aula07-billboard/log-duracao.png){.aula07-figure-large fig-alt="Comparação da duração das músicas após adicionar uma observação didática de 45 minutos. Na escala original, o valor extremo comprime as demais músicas; na escala logarítmica, a cauda é comprimida."}

**Observação:** adicionamos artificialmente uma música de **45 minutos** apenas
para tornar visível o efeito da transformação.

Como o log é monotônico, ele preserva a ordem dos valores.

Na escala original, 45 minutos comprime as músicas usuais entre 2 e 5 minutos.
No log, distâncias representam razões, e o valor extremo ocupa menos espaço.
...
![](assets/aula07-billboard/popularidade.png){.aula07-figure-large fig-alt="Histograma da popularidade das músicas com linhas de média e mediana."}
