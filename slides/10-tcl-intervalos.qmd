...
format:
  revealjs:
    ...
    chalkboard: true
    preview-links: auto
    footer: '<span class="lecture-footer-label">ICD: Aula 10 — Teorema Central do Limite</span><a class="lecture-footer-button" href="../aulas/10-tcl-intervalos.html">voltar para a Aula</a>'
execute:
  enabled: false
  echo: false
---
...
![](assets/aula10-tcl-inferencia/amostra-entrega.png){.wide-chart fig-alt="Dois histogramas de densidade lado a lado, construídos com amostras independentes de 30 tempos de entrega da mesma população Olist, cada um acompanhado por sua densidade estimada por KDE."}
...
## LGN ou TCL? {.quiz-question}

Qual afirmação descreve corretamente o Teorema Central do Limite?

- [A média amostral se aproxima de $\mu$, sem informar o formato de sua distribuição]{data-explanation="Essa é a mensagem principal da Lei dos Grandes Números."}
- [O erro padronizado da média se aproxima de uma Normal padrão]{.correct data-explanation="Sob condições adequadas, (X̄−μ)/(σ/√n) converge em distribuição para N(0,1)."}
- [Os dados individuais tornam-se Normais quando $n$ cresce]{data-explanation="O TCL descreve a distribuição da média, não transforma as observações originais."}
- [A independência e a variância finita são irrelevantes]{data-explanation="As condições do teorema determinam quando a aproximação pode ser usada."}
...
![](assets/aula10-tcl-inferencia/cauda-pesada-amostras-20-100-large.png){.heavy-tail-three-panel fig-alt="Três painéis. O primeiro mostra uma população lognormal fortemente assimétrica e com cauda pesada. O segundo mostra uma amostra aleatória de vinte observações que parece quase simétrica e não contém valores da cauda longa. O terceiro mostra uma amostra de cem observações na qual a assimetria e a cauda ficam visíveis."}
...
![](assets/aula10-tcl-inferencia/tirinha-gps-confianca.png){.confidence-gps-analogy fig-alt="Tirinha em dois quadros com mapas idênticos. No primeiro, um pino marca uma localização e representa a estimativa pontual. No segundo, um círculo ao redor do pino representa a região de incerteza associada ao intervalo de confiança."}

::: {.statement .confidence-gps-takeaway}
...
## Uma previsão infalível {.bugs-forecast-slide}

![](assets/aula10-tcl-inferencia/tirinha-intervalo-prazo-codigo.png){.bugs-forecast-comic fig-alt="Tirinha em três quadros. Uma gerente pergunta quanto tempo falta para o código ficar pronto. Um desenvolvedor responde que levará entre cinco minutos e seis meses. No último quadro, ele conclui: cobertura excelente, precisão nem tanto."}
...
## O que significa 95% de confiança? {.quiz-question}

Qual interpretação frequentista está correta?

- [Há 95% de probabilidade de $\mu$ estar no intervalo já calculado]{data-explanation="Após o cálculo, o intervalo específico cobre ou não cobre o parâmetro fixo."}
- [Em repetições, cerca de 95% dos intervalos construídos pelo procedimento cobrem $\mu$]{.correct data-explanation="A confiança é uma propriedade de cobertura do procedimento em amostras repetidas."}
- [95% dos dados individuais ficam dentro do intervalo]{data-explanation="O intervalo estima a média populacional, não delimita 95% das observações."}
- [A confiança garante ausência de viés de seleção]{data-explanation="Um intervalo correto não repara uma amostra que não representa a população."}
...
![](assets/aula10-tcl-inferencia/margem-tamanho-ate-1400.png){fig-alt="Curva azul decrescente da margem de erro em função do tamanho da amostra, prolongada até a interseção correspondente a aproximadamente 1400 observações e margem de meio dia."}
...
::: {.icd-grid .paired-samples-grid}
::: {.icd-card}
**Por que usamos $t^*$?**

Como $\sigma_D$ é desconhecido, usamos $s_D$. Sob diferenças Normais,
$T=(\bar D-\mu_D)/(s_D/\sqrt n)$ segue $t_{n-1}$, que tem caudas mais pesadas.
:::
...
## Exemplo numérico pareado {.smaller}
...
$$
\bar D=-1{,}20,\qquad s_D=1{,}48,
\qquad \operatorname{SE}(\bar D)=\frac{1{,}48}{\sqrt5}=0{,}66.
$$
