...
format:
  revealjs:
    ...
    chalkboard: true
    preview-links: auto
    footer: '<span class="lecture-footer-label">ICD: Aula 09 — Escolhendo estimadores</span><a class="lecture-footer-button" href="../aulas/09-risco-estimadores.html">voltar para a Aula</a>'
execute:
  enabled: false
  echo: false
---
...
## Perdas podem ser assimétricas {.smaller}
...
::: {.callout-note title="Exemplo completo: dados 1, 2 e 3"}
Escolhemos uma estimativa $a$ para representar $x_1=1$, $x_2=2$ e $x_3=3$.
Com perda quadrática simétrica,

$$
R(a)=\sum_{i=1}^{3}(a-x_i)^2,
\qquad R'(a)=2(a-1)+2(a-2)+2(a-3)=6a-12.
$$
...
## Qual centro minimiza cada perda? {.quiz-question .quiz-multiple}

Selecione as associações corretas.

- [A média minimiza a soma dos erros quadráticos]{.correct data-explanation="Ao zerar a derivada da perda quadrática, obtemos o equilíbrio aritmético θ = x̄."}
- [A mediana minimiza a soma dos erros absolutos]{.correct data-explanation="Na mediana, há equilíbrio entre observações à esquerda e à direita."}
- [A mediana sempre minimiza a perda quadrática]{data-explanation="A perda quadrática é minimizada pela média, salvo coincidências particulares entre média e mediana."}
- [A escolha da perda não altera o estimador preferido]{data-explanation="Perdas diferentes formalizam custos diferentes e podem selecionar estimadores distintos."}
...
::: {.columns .concept-cards .estimation-methods}
::: {.column width="31%"}
### Momentos
...
$$
\operatorname{MAE}(\widehat\theta)
=E\left[|\widehat\theta-\theta|\right].
$$
...
\begin{aligned}
\widehat\theta-\theta
&=(\widehat\theta-m)+(m-\theta),\\[2mm]
E[(\widehat\theta-\theta)^2]
&=E[(\widehat\theta-m)^2]
+2(m-\theta)E[\widehat\theta-m]
+(m-\theta)^2,\\
&=\operatorname{Var}(\widehat\theta)+0+B(\widehat\theta)^2.
\end{aligned}
$$

O termo cruzado desaparece porque $E[\widehat\theta-m]=0$; além disso,
$B(\widehat\theta)=m-\theta$.

::: {.statement}
Risco = variância + viés².
:::

## Como interpretar o MSE? {.quiz-question}

Um estimador tem viés pequeno, mas variância muito alta. O que a decomposição do MSE implica?

- [O MSE precisa ser pequeno porque o viés é pequeno]{data-explanation="O termo de variância também contribui para o erro quadrático médio."}
- [O MSE combina viés ao quadrado e variância]{.correct data-explanation="A decomposição é MSE = viés² + variância, sob perda quadrática."}
- [Variância e viés sempre diminuem juntos]{data-explanation="Frequentemente existe um trade-off: reduzir um componente pode aumentar o outro."}
- [O MSE mede apenas robustez a outliers]{data-explanation="Robustez é outra propriedade; o MSE resume desempenho quadrático sob uma distribuição."}
