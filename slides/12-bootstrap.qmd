...
- retomar o teste A/B da Aula 11 com bootstrap;
- reconhecer quando o bootstrap é — e não é — válido.

## Airbnb NYC 2019 {.smaller}

::: {.columns}
::: {.column width="42%"}
- 48.895 anúncios em Nova York em 2019;
- unidade de observação: **um anúncio**;
- dados públicos do Kaggle sob licença CC0;
- preço anunciado: não observamos se a estadia ocorreu.

Fonte: `dgomonov/new-york-city-airbnb-open-data`.
:::
...
| `price` | preço anunciado por noite |
...
Para as análises seguintes, removemos 11 preços iguais a zero e o 1% superior. Permanecem 48.410 anúncios entre US$ 1 e US$ 799; essa limpeza muda explicitamente a população-alvo.
...
## A amostra observada {.smaller}

Para tornar a comparação mais justa, usamos acomodações inteiras de Manhattan e Brooklyn, preços entre US$ 1 e US$ 799 e uma amostra didática de 600 anúncios por distrito.
...
## A densidade na mediana controla a precisão {.smaller}
...
## A amostra vira uma população provisória {.smaller}

Uma estimativa não mostra sua estabilidade: se repetíssemos a coleta, obteríamos exatamente US$ 40 outra vez? Precisamos imaginar muitas amostras semelhantes, mas só observamos uma.
...
## Exemplo: quantas observações distintas aparecem? {.smaller}

::: {.callout-note title="Uma réplica com n = 8"}
...
## O bootstrap em notação formal {.smaller}
...
$n$ é o tamanho de cada amostra bootstrap e $R$ é o número de réplicas. Aumentar $R$ reduz o erro de simulação; aumentar $n$ reduz a incerteza estatística.
...
## O bootstrap também pode estimar viés {.smaller}
...
## O intervalo percentil é direto {.smaller}
...
Os 95% centrais vão de **US$ 30 a US$ 51** e permanecem acima de zero.
:::
:::
...
O cálculo detalhado do BCa e um exercício resolvido permanecem no material de estudo como leitura complementar.
...
No exemplo didático da Olist, 5.000 pedidos foram atribuídos a cada processo:

| Grupo | Processo | Atrasos | Taxa |
|---|---|---:|---:|
| A | processo atual | 415 | 8,3% |
| B | alerta preventivo | 345 | 6,9% |

$$
\widehat\Delta=\widehat p_B-\widehat p_A=-0{,}014.
$$

Testamos $H_0:\Delta=0$ contra $H_A:\Delta<0$. Valores negativos favorecem o alerta.

::: {.callout-note title="Natureza do exemplo"}
A base histórica da Olist é observacional. Os grupos A e B são uma simulação didática de um experimento randomizado no mesmo contexto.
:::

## Bootstrap usual para o efeito do alerta {.smaller}

Primeiro representamos **cada pedido** por 1 (atraso) ou 0 (no prazo). Em cada réplica, sorteamos 5.000 pedidos **com reposição** dentro de A e dentro de B:

$$
\mathbf y_A=(\underbrace{1,\ldots,1}_{415},\underbrace{0,\ldots,0}_{4.585}),
\qquad
\mathbf y_B=(\underbrace{1,\ldots,1}_{345},\underbrace{0,\ldots,0}_{4.655}).
$$

Calculamos $\Delta^*=\overline y_B^*-\overline y_A^*$ em cada réplica. Assim, o código executa literalmente o algoritmo bootstrap estudado na aula.

Com $R=50.000$ réplicas, o intervalo percentil de 95% é

$$
IC_{95\%}(\Delta)=[-0{,}0242,-0{,}0038]
=[-2{,}42,-0{,}38]\text{ p.p.}
$$

O intervalo não contém zero. O bootstrap usual reproduz a diferença observada e descreve sua incerteza.

<details class="code-fold"><summary>Código do intervalo</summary>

```python
rng = np.random.default_rng(2026)
resultados_a = np.r_[np.ones(415), np.zeros(4585)]
resultados_b = np.r_[np.ones(345), np.zeros(4655)]
boot_effect = np.empty(50_000)
for r in range(50_000):
    a_star = rng.choice(resultados_a, 5000, replace=True)
    b_star = rng.choice(resultados_b, 5000, replace=True)
    boot_effect[r] = b_star.mean() - a_star.mean()
ci_ab = np.quantile(boot_effect, [0.025, 0.975])
```
</details>

## Bootstrap sob $H_0$: reamostre a amostra combinada {.smaller}

Para construir o mundo nulo, os dois grupos precisam compartilhar a mesma taxa:

$$
\widehat p_{pool}=\frac{415+345}{5000+5000}=0{,}076.
$$

Juntamos os 10.000 resultados observados — 760 uns e 9.240 zeros. Em cada réplica, retiramos **com reposição** dois novos grupos de 5.000 pedidos dessa amostra combinada. Como ambos vêm da mesma distribuição empírica, $H_0$ é verdadeira no mundo simulado.

::: {.statement}
A diferença essencial é a origem do sorteio: no bootstrap usual, cada grupo usa seus próprios dados; sob $H_0$, os dois grupos usam a mesma amostra combinada.
:::

## Valor-p por reamostragem sob $H_0$ {.smaller}

Contamos quantas diferenças reamostradas são menores ou iguais ao efeito observado, $-0{,}014$:

$$
\widehat p_{boot}
=\frac{1+\sum_{b=1}^{R}\mathbb 1\{\Delta_b^{*(0)}\leq-0{,}014\}}{R+1}
=\frac{1+217}{50.001}\approx0{,}0044.
$$

Essa construção preserva os valores realmente observados (0 e 1) e torna explícita a reamostragem dos dados, sem recorrer diretamente à distribuição Binomial.

<details class="code-fold"><summary>Código do teste bootstrap</summary>

```python
resultados_pool = np.r_[resultados_a, resultados_b]
boot_null = np.empty(50_000)
for r in range(50_000):
    a0 = rng.choice(resultados_pool, 5000, replace=True)
    b0 = rng.choice(resultados_pool, 5000, replace=True)
    boot_null[r] = b0.mean() - a0.mean()
p_boot_ab = (1 + np.sum(boot_null <= -0.014)) / 50_001
```
</details>

## Duas distribuições, duas perguntas

![](assets/aula12-bootstrap/ab-olist-bootstrap.png){height=355 fig-alt="À esquerda, distribuição bootstrap usual da diferença de atrasos centrada em menos 1,4 ponto percentual e intervalo de 95 por cento. À direita, distribuição bootstrap sob a hipótese nula centrada em zero, com a cauda além do efeito observado destacada."}

O bootstrap usual fornece o intervalo do efeito. O bootstrap sob $H_0$ fornece $p\approx0{,}0044$, muito próximo de $0{,}0041$ obtido pelo teste $z$ na Aula 11. Aqui, ambos os resultados vêm da reamostragem direta das linhas 0/1; a pequena diferença vem do erro de Monte Carlo.

<details class="code-fold"><summary>Código do gráfico</summary>

```python
axes[0].hist(100 * boot_effect, bins=55)
axes[1].hist(100 * boot_null, bins=55)
axes[1].hist(100 * boot_null[boot_null <= -0.014], bins=20)
for ax in axes:
    ax.axvline(-1.4, color="darkorange")
    ax.axvline(0, color="navy", linestyle="--")
```
</details>

## Validade significa cobertura em repetições {.smaller}
...
![](assets/aula12-bootstrap/cobertura-bootstrap.png){height=285 fig-alt="Vinte intervalos bootstrap obtidos de amostras repetidas, comparados à diferença de medianas na população didática."}

$P_F$ representa amostras repetidas de $F$ e $1-\alpha$ é a cobertura nominal. Na prática, a população verdadeira não está disponível para essa verificação.
...
## Condições e limites do bootstrap {.smaller}

::: {.columns .concept-cards}
::: {.column width="48%"}
### Costuma funcionar quando

- a amostra representa a população-alvo;
- o esquema respeita a dependência;
- a estatística varia regularmente;
- há dados perto da característica estimada;
- a reamostragem imita a coleta.
:::
...
- eventos ausentes permanecem ausentes;
- poucos dados não representam caudas raras;
- outliers podem dominar réplicas;
- estatísticas irregulares podem falhar.
:::
:::
...
## Dependência e unidade de reamostragem

![](assets/aula12-bootstrap/dependencia-host.png){height=330 fig-alt="Distribuição do número de anúncios por anfitrião e comparação entre bootstrap por anúncio e por anfitrião."}

Anúncios do mesmo anfitrião podem compartilhar preço, localização e decisões de gestão. Reamostrar anúncios como independentes pode subestimar a incerteza. Quando a dependência ocorre por anfitrião, reamostramos **anfitriões inteiros**.
...
## Duas conclusões substantivas {.smaller}
...
- redução observada de 1,4 p.p.;
- IC bootstrap de 95%: **−2,42 a −0,38 p.p.**;
- teste bootstrap unilateral: **$\widehat p\approx0{,}0044$**;
- resultado próximo do teste $z$ da Aula 11.

O experimento é uma simulação didática no contexto da base Olist.
:::
:::
...
6. refaça o A/B com 10.000 e 50.000 réplicas e compare o erro de Monte Carlo;
7. transforme o teste A/B em bilateral e interprete a mudança no valor-p.
